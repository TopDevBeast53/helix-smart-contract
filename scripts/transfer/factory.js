 ...
 *     npx hardhat run scripts/transfer/factory.js --network ropsten
 */
...
const contractName = "HelixFactory"

/// The name of the contract in the contracts mapping
const contractAddress = contracts.factory[env.network]
...
const transferTimelockOwner = false
...
async function setMultiSigOwner(newOwnerAddress) {
    ...
    let tx = await contract.setFeeToSetter(newOwnerAddress, overrides)
    await tx.wait()
}
