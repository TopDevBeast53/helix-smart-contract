...
contract MultiSigWallet {
    ...
    event SetMaster(address indexed caller, address indexed master);
    ...
    /// Required confirmation for transaction execution
    ...
    // mapping from tx index => true if master has confirmed and false otherwise
    mapping(uint256 => bool) public masterConfirmed;
    ...
    error MasterHasNotConfirmed(uint256 txIndex);
    ...
    modifier masterHasConfirmed(uint256 _txIndex) {
        if (!masterConfirmed[_txIndex]) revert MasterHasNotConfirmed(_txIndex);
        _;
    }
        ...
        notExecuted(_txIndex)
    {
        ...
        if (msg.sender == master) {
            masterConfirmed[_txIndex] = false;
        }
