...
import '../interfaces/IAuraExchangeFactory.sol';

contract AuraMigrator is IAuraMigrator {
    IAuraExchangeFactory immutable exchangeFactory;
    ...
    constructor(address _exchangeFactory, address _router) {
        exchangeFactory = IAuraExchangeFactory(_exchangeFactory);
    ...
    function migrate(address token, address to, uint tokenAmountMin, uint ethAmountMin, uint deadline) external override {
        IAuraExchange exchange = IAuraExchange(exchangeFactory.getExchange(token));
